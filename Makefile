...
build-frontend:
	docker --log-level=debug build --pull --file=frontend/docker/production/nginx/Dockerfile --tag=${REGISTRY}/auction-gateway:${IMAGE_TAG} frontend
build-api:
	docker --log-level=debug build --pull --file=gateway/docker/production/php-fpm/Dockerfile --tag=${REGISTRY}/auction-gateway:${IMAGE_TAG} api
